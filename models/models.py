...
class base_branches(nn.Module):
    def __init__(self, backbone="ibn", stride=1):
        ...
        # encoder
        trans_layer = nn.TransformerEncoderLayer(d_model=1024, nhead=1, dropout=0.1, activation="relu" # type: ignore
                                                 )
        self.trans_encoder = nn.TransformerEncoderLayer(trans_layer, num_layers=1)
